...
struct Cloth {
  ...
  void stimulate(double frames_per_sec, double simulation_steps, ClothParameters *cp,
                vector<Vector3D> external_accelerations,
                vector<CollisionObject *> *collision_objects, double gravity_y);
  void stimulate_sine(double frames_per_sec, double simulation_steps, ClothParameters *cp,
                vector<Vector3D> external_accelerations,
                vector<CollisionObject *> *collision_objects, double gravity_y);
  ...
  int idx(int y, int x);
  void waterDrop(int y, int x, double displacement);
  void reset_raindrop();
  void rippleSpread(double frames_per_sec, double simulation_steps, ClothParameters *cp);
  void ripple(PointMass& pm, double displacement);
  bool OutOfBound(int y, int x);
  void variedWaterDrop(int y, int x, int layers, double displacement, double ratio);
