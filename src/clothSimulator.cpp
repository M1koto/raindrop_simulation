...
//    m_project_root + "/textures/cube/negz.jpg"
    ...
    m_project_root + "/textures/cube/sky.jpg",
    m_project_root + "/textures/cube/sky.jpg",
    m_project_root + "/textures/cube/sky.jpg",
    m_project_root + "/textures/cube/sea.jpg",
    m_project_root + "/textures/cube/sky.jpg",
    m_project_root + "/textures/cube/sky.jpg"
  };
...
//    }
    // create new function for water rippling
    // current rain drop pos
    // hard coded
...
void ClothSimulator::initGUI(Screen *screen) {
  ...
  new Label(window, "Raindrop Position", "sans-bold");

  {
    ...
    fb->setCallback([this](float value) { raindrop_position.x = value; });
    ...
    fb->setFontSize(14);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { raindrop_position.y = value; });
    ...
    fb->setFontSize(14);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { raindrop_position.z = value; });
    ...
    fb->setFontSize(14);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { raindrop_radius = value; });

    Button *b = new Button(window, "Create Raindrop");
    b->setFlags(Button::ToggleButton);
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) {
         Vector3D origin = Vector3D(raindrop_position.x, raindrop_position.z, raindrop_position.y);
         Sphere *s = new Sphere(origin, raindrop_radius, 0.03, 40, 40);
          // Vector3D origin = Vector3D(0.5, 0.2, 0.5);
          // Sphere *s = new Sphere(origin, 0.02, 0.03, 40, 40);
          collision_objects->push_back(s);
          is_paused = true;
          this->drawContents();
        });
    Button *reset_b = new Button(window, "Clear Raindrop");
    reset_b->setFlags(Button::ToggleButton);
    reset_b->setFontSize(14);
    reset_b->setChangeCallback(
        [this](bool state) {
          cloth->reset_raindrop();
          this->collision_objects->clear();
        });
    Button *random_b = new Button(window, "Random Raindrop!");
    random_b->setFlags(Button::ToggleButton);
    random_b->setFontSize(14);
    random_b->setChangeCallback(
        [this](bool state) {
          int count = 0;
          while (count < 10) {
            if (true) { // random sample
              double x = ((double)rand()/RAND_MAX)*(1.0-0.1)+0.1;
              double z = ((double)rand()/RAND_MAX)*(1.0-0.1)+0.1;
              double y = ((double)rand()/RAND_MAX)*(0.9-0.1)+0.1;
              double raindrop_radius = ((double)rand()/RAND_MAX)*(0.02-0.01)+0.01;
              //std::cout << raindrop_radius << std::endl;
